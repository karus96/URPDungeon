        ...
        void DisplayModOverridesGUI(FieldModification mod, FieldSetupComposition.ModOverrideHelper modOverrides, ref Vector2 pfScroll)
        {
            ...
            if (mod.DrawSetupFor == FieldModification.EModificationMode.CustomPrefabs)
            {
                ...
                for (int p = 0; p < pList.Count; p++)
                {
                    ...
                    if (modOverrides != null)
                        if (modOverrides.OverridePrefabs != null)
                            if (p < modOverrides.OverridePrefabs.Count)
                                ...
                                {
                                    if (modOverrides.OverridePrefabs[p].Prefab != null)
                                    {
                    ...
                    PrefabReference.DrawPrefabField(pList[p], Color.gray, "Default", 54, () => { EditorGUIUtility.PingObject(pList[p].Prefab); }, null, true, null, false, false);
                    ...
                    if (modOverrides != null)
                        if (modOverrides.OverridePrefabs != null)
                            if (p < modOverrides.OverridePrefabs.Count)
                            {
                                ...
                                modOverrides.OverridePrefabs[p].Prefab = (GameObject)EditorGUILayout.ObjectField(modOverrides.OverridePrefabs[p].Prefab, typeof(GameObject), false, wdth);
                            }

                    if (prref != null)
                    {
                        if (prref.Prefab == null)
                            PrefabReference.DrawPrefabField(prref, Color.gray, "_", 54, () => { EditorGUIUtility.PingObject(prref.Prefab); }, null, true, null, false);
                        else
                            PrefabReference.DrawPrefabField(prref, Color.gray, "_", 54, () => { EditorGUIUtility.PingObject(prref.Prefab); }, () => { modOverrides.OverridePrefabs[p].Prefab = null; }, true, null, false);
                    }
