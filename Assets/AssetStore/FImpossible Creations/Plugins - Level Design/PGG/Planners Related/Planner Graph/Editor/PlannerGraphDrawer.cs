...
namespace FIMSpace.Graph
{
    ...
    public class PlannerGraphDrawer : FGraphDrawerBase
    {
        ...
        public List<PlannerRuleBase> Nodes { get { if (DisplayMode == FieldPlanner.EViewGraph.Procedures_Placement) return currentSetup.Procedures; else return currentSetup.PostProcedures; } }
        ...
        protected override Vector3 DrawConnectionWithTangents(Vector2 start, Vector2 end, bool outToInput = true, Color? color = null, float thickness = 5, bool getMiddlePoint = false, Texture2D lineTex = null, FGraph_NodeBase_Drawer.EConnectorsWireMode wireMode = FGraph_NodeBase_Drawer.EConnectorsWireMode.Left_Right, float oXTan = 0.8f, float iXTan = 0.8f, float oYTan = 0.6f, float iYTan = 0.6f)
        {
            ...
            {
                if (end.y < start.y)
                    return base.DrawConnectionWithTangents(start, end, outToInput, color, thickness * 0.6f, getMiddlePoint, null, wireMode);
                else
                    return base.DrawConnectionWithTangents(start, end, outToInput, color, thickness * 0.6f, getMiddlePoint, null, wireMode, 0.9f, 0.5f, 1.05f, 0.6f);
            }
            ...
            if (end.y > start.y)
            {
                return base.DrawConnectionWithTangents(start, end, outToInput, color, thickness, getMiddlePoint, PlannerGraphWindow.PlannerGraphStyles.TEX_Gradient1, wireMode);
            }
            else // Direct to above
            {
                if ((end.x > start.x) && _DrawConnection_input && _DrawConnection_output)
                {
                    ...
                    base.DrawConnectionWithTangents(nend, end, outToInput, new Color(0.7f, 0.7f, 0.7f, 0.35f), thickness, getMiddlePoint, PlannerGraphWindow.PlannerGraphStyles.TEX_Gradient1, wireMode, 0.4f, 1.0f, -0.25f, -0.1f);
                    return base.DrawConnectionWithTangents(start, nend, outToInput, color, thickness, getMiddlePoint, PlannerGraphWindow.PlannerGraphStyles.TEX_Gradient1, FGraph_NodeBase_Drawer.EConnectorsWireMode.Left_Right, 0.8f, 0.9f, 0.0f, -0.3f);
                }
                else
                    return base.DrawConnectionWithTangents(start, end, outToInput, color, thickness, getMiddlePoint, PlannerGraphWindow.PlannerGraphStyles.TEX_Gradient1, wireMode);
            }
        ...
        protected override void DrawGraphOverlay()
        {
            ...
            Rect up = new Rect(0f, graphTopMarginOffset - 3, 24, 20);
            Rect r = new Rect(up);
            ...
            if (TopTitle != "")
            {
                ...
                Rect titleR = new Rect(centerX - 250f, graphTopMarginOffset - 3, 500f, 38f);
