    ...
    public partial class TileMeshSetup
    {
        ...
        public Mesh FullGenerateMesh()
        {
            if (GenTechnique == EMeshGenerator.CustomMesh)
            {
                ...
                LatestGeneratedMesh = GameObject.Instantiate(CustomMesh);
        ...
        private Mesh GenerateMesh()
        {
            ...
            if (GenTechnique == EMeshGenerator.Lathe)
                ...
                LatestGeneratedMesh = GenerateLoft(previewShape, previewShape2, new Vector3(width, height, (width + height) * _loftDepthCurveWidener), HardNormals, UVMul, UVFit, NormalsMode, _loft_depthDim);
            else if (GenTechnique == EMeshGenerator.Extrude)
                LatestGeneratedMesh = GenerateExtrude(previewShape, new Vector3(width, height, depth), _extrudeFrontCap, _extrudeBackCap, HardNormals, UVMul, UVFit, NormalsMode);
        ...
        public Mesh GenerateLoft(List<MeshShapePoint> shape, List<MeshShapePoint> shapeDistrib, Vector3 dimensions, float hardNormals, Vector2 uvMul, EUVFit uvFit, ENormalsMode normalsCompute, float distribDepth)
        {
        ...
        public Mesh GenerateExtrude(List<MeshShapePoint> shape, Vector3 dimensions, bool extrudeFrontCap, bool extrudeBackCap, float hardNormals, Vector2 uvMul, EUVFit uvFit, ENormalsMode normalsCompute)
        {
            if (uvMul.x == 0f) uvMul.x = 1f;
            if (uvMul.y == 0f) uvMul.y = 1f;
