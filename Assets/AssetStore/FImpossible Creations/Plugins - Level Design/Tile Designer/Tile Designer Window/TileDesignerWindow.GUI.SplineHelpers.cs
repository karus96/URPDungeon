    ...
    public partial class TileDesignerWindow
    {
        ...
        void AddNewPointInCanvasPos(Rect displayArea, List<CurvePoint> list, Event e)
        {
            ...
            else
            {
                ...
                if (ind > 0)
                {
                    dotPre = Vector2.Dot((pos - list[ind].Pos).normalized,
                        (list[ind - 1].Pos - list[ind].Pos).normalized
                        );
                }

                if (ind < list.Count - 1)
                {
                    dotNext = Vector2.Dot((pos - list[ind].Pos).normalized,
                        (list[ind + 1].Pos - list[ind].Pos).normalized
                        );
                }
        ...
        CurvePoint FindNearestInCanvasPoint(Vector2 pos, List<CurvePoint> list)
        {
            ...
            {
                var p = list[i];
                float dist = Vector2.Distance(pos, p.Pos);
        ...
        void DrawCurves(Rect r, List<CurvePoint> curvePoints, bool mirror = false, bool secondary = false)
        {
            if (!secondary)
