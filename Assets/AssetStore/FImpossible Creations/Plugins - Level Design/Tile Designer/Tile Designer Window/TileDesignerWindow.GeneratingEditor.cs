...
namespace FIMSpace.Generating
{
    public partial class TileDesignerWindow
    {
        ...
        private void GUIEditor()
        {
            ...
            if (!_foldout_bigPreview) if (width > 0f && height > 0f && depth > 0f && EditedTileSetup._loftDepthCurveWidener > 0f)
                {
                    PrepareCurveDraw();
        ...
        bool DisplaySplineHandler(Rect r, List<CurvePoint> list, bool? secondary)
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                Vector2 initRectPos = p.Pos; // To change check
                ...
                if (_latestEditorDisplayRect.width > 10 && _latestEditorDisplayRect.height > 10)
                    if (p.Pos != initRectPos)
                    {
                        ...
                        if (gridSnapping > 0f)
                        {
                            ...
                            if (sPos != p.Pos) p.rPos = sPos;
                        }

                        float sx = s - r.x;
                        float sy = s - r.y;

                        if (p.Pos.x < -sx) p.rPos = new Vector2(-sx, p.Pos.y);
                        if (p.Pos.x > r.width - sx) p.rPos = new Vector2(r.width - sx, p.Pos.y);
                        if (p.Pos.y < -sy) p.rPos = new Vector2(p.Pos.x, -sy);
                        if (p.Pos.y > r.height - sy) p.rPos = new Vector2(p.Pos.x, r.height - sy);
        ...
        void DrawSubdivsPreviewLines(List<MeshShapePoint> previewShape, float width, float height, bool mirror)
        {
            Color preC = GUI.color;
